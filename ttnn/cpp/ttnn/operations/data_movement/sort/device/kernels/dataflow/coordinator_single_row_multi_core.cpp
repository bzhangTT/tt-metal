...
void kernel_main() {
    ...
    experimental::CircularBuffer input_tensor_cb(input_tensor_cb_index);
    experimental::CircularBuffer index_tensor_cb(index_tensor_cb_index);
    experimental::CircularBuffer rm_coord_value_row(rm_coord_value_row_cb);
    experimental::CircularBuffer rm_coord_index_row(rm_coord_index_row_cb);
        ...
        for (uint32_t w = 0; w < Wt; w++) {
            ...
            if constexpr (is_row_major) {
                for (uint32_t row = 0; row < TILE_H; row++) {
                    ...
                    noc.async_write(
                        experimental::use<experimental::CircularBuffer::AddrSelector::WRITE_PTR>(rm_coord_value_row),
                ...
                for (uint32_t row = 0; row < TILE_H; row++) {
                    noc.async_write(
                        experimental::use<experimental::CircularBuffer::AddrSelector::WRITE_PTR>(rm_coord_index_row),
