...
std::vector<Tensor> sort(
    ...
    if (optional_output_tensors.has_value()) {
        ...
        TT_FATAL(
            out0_mem == out1_mem,
            "Preallocated output tensors must have the same memory config. "
            "Got values memory_config: {} and indices memory_config: {}",
            out0_mem,
            out1_mem);
    ...
    auto sorted_tensors =
        ttnn::prim::sort(padded_input_tensor, dim, descending, stable, device_op_mem_cfg, output_tensors);
    ...
    // Skip when there was a preallocated layout mismatch (RM preallocated with TILE
    // input): that path converts to RM layout below instead.
    const bool explicit_sharded_output = sort_mem_cfg.is_sharded() && !preallocated_layout_mismatch;
    if (explicit_sharded_output) {
    ...
    // user's handle receives data in the expected format.
