...
element format, but possibly with a restricted range. The width of this type
...
.. code-block:: c++
    ...
    ToType convert (FromType in, RoundMode::Stochastic rounding)
    template<typename ToType, typename FromType>
    ToType convert (FromType in, unsigned descale, RoundMode::Stochastic rounding)

Returns the rounded value performing round-to-even when ''rounding''
is ''RoundMode::RoundToEven'', stochastic rounding when ''rounding'' is
''RoundMode::RoundStochastic'', and round-to-zero when it is
''RoundMode::RoundZero'' (not available on WormHole).  Not all
