...
```cpp
const uint32_t noc_max_burst_size = tt::tt_metal::hal::get_noc_max_burst_size();
...
Do not hardcode the architecture split in the conv factory. Device kernels
already use `NOC_MAX_BURST_SIZE`. The host side should use an existing shared
host accessor if one is available. If it is not available yet, add one to HAL
or another shared Metal utility first, then use it from conv. The
`hal::get_noc_max_burst_size()` name above is the intended shape of that
accessor, not a claim that this exact function exists today. Adding it would
also give nearby code such as
`dram_sharded_helpers::get_max_page_size_and_num_pages` a path away from its
local TODO/hardcoding.
...
4. NOC burst size portability.
   ...
   duplicate per-arch constants in the conv factory. Use an existing shared
   host accessor if present; otherwise add one to HAL/shared Metal utilities as
   a prerequisite.
...
1. Add or locate a shared host accessor for `NOC_MAX_BURST_SIZE`.
