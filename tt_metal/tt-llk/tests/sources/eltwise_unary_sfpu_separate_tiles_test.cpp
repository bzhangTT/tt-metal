...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    for (int block = 0; block < params.NUM_BLOCKS; block++)
    {
        ...
        // Run SFPU: read from DST_INDEX_IN, write to DST_INDEX_OUT
        _llk_math_eltwise_unary_sfpu_start_<DST_SYNC>(DST_INDEX_IN);
        test_utils::call_sfpu_operation<APPROX_MODE, is_fp32_dest_acc_en, iterations, FAST_MODE, false, CLAMP_NEGATIVE>(
            SFPU_UNARY_OPERATION, DST_INDEX_IN, DST_INDEX_OUT, formats.math);
...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    for (int block = 0; block < params.NUM_BLOCKS; block++)
    {
        ...
        _llk_pack_<DST_SYNC, is_fp32_dest_acc_en, false>(DST_INDEX_OUT, L1_ADDRESS(params.buffer_Res[block]));
