...
inline void _calculate_binary_max_min_sfp_rows_(const std::uint32_t offset0, const std::uint32_t offset1, const std::uint32_t offset2, const int d)
{
    constexpr std::uint32_t load_mode = IS_FP16A ? p_sfpu::sfpmem::FP16A : p_sfpu::sfpmem::DEFAULT;
    TT_SFPLOAD(p_sfpu::LREG0, load_mode, ADDR_MOD_7, 0 /* done */, offset0 + (d << 1)); // load FP row from in0
    TT_SFPLOAD(p_sfpu::LREG1, load_mode, ADDR_MOD_7, 0 /* done */, offset1 + (d << 1)); // load FP row from in1
    TTI_SFPNOP(0 /* srcs_wr_done */, 0 /* srcs_rd_done */, 0 /* dest_done */);          // ensure LREG1 write is visible before SFPSWAP reads it
    ...
    TT_SFPSTORE(IS_MAX_OP ? p_sfpu::LREG1 : p_sfpu::LREG0, load_mode, ADDR_MOD_7, 0 /* done */, offset2 + (d << 1)); // store max (LREG1) or min (LREG0)
}
...
#pragma GCC unroll 8
    ...
    {
        _calculate_binary_max_min_sfp_rows_<IS_MAX_OP, IS_FP16A>(offset0, offset1, offset2, d);
    }
}
...
template <bool IS_MAX_OP = true, bool IS_UNSIGNED = false>
inline void _calculate_binary_max_min_int32_sfp_rows_(const std::uint32_t offset0, const std::uint32_t offset1, const std::uint32_t offset2, const int d)
{
    TT_SFPLOAD(p_sfpu::LREG0, p_sfpu::sfpmem::INT32, ADDR_MOD_7, 0 /* done */, offset0 + (d << 1)); // load INT32 row from in0
    TT_SFPLOAD(p_sfpu::LREG1, p_sfpu::sfpmem::INT32, ADDR_MOD_7, 0 /* done */, offset1 + (d << 1)); // load INT32 row from in1
    ...
    TT_SFPSTORE(IS_MAX_OP ? p_sfpu::LREG1 : p_sfpu::LREG0, p_sfpu::sfpmem::INT32, ADDR_MOD_7, 0 /* done */, offset2 + (d << 1)); // store INT32 result
}
...
#pragma GCC unroll 8
    for (int d = 0; d < ITERATIONS; d++)
    {
        _calculate_binary_max_min_int32_sfp_rows_<IS_MAX_OP, IS_UNSIGNED>(offset0, offset1, offset2, d);
    }
}
